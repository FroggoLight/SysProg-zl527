...
int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        if (rc == OK_EXIT) {
            printf("Server requested to stop.\n");
            break;
        } else if (rc == OK) {
...
int exec_client_requests(int cli_socket) {
    ...
    while ((bytes_received = recv(cli_socket, io_buff, RDSH_COMM_BUFF_SZ - 1, 0)) > 0) {
        ...
        if (strchr(io_buff, '|') != NULL) {
            ...
            build_cmd_rc = build_cmd_list(io_buff, &clist);
            ...
            if (result == BI_CMD_EXIT) {
                ...
                if (rsh_match_command(clist.commands[0].argv[0]) == BI_CMD_CD) {
                    if (clist.commands[0].argc > 1) {
                        send_message_string(cli_socket, "Directory Changed Successfully.\n");
                    } 
                    else {
                        send_message_string(cli_socket, "cd usage: cd <directory>\n");
                    }
                }
            } 
                ...
                if (clist.commands[0].argv[0] != NULL) {
                    child_pid = fork();
                    if (child_pid == -1) {
                        perror("fork failed");
                    } 
...
int send_message_string(int cli_socket, char *buff){
    ...
    // Send the EOF character to indicate completion
    if (send_message_eof(cli_socket) != OK) {
        return ERR_RDSH_COMMUNICATION;
    }

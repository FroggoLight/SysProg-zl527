...
{
    if (lseek(fd, id*STUDENT_RECORD_SIZE, SEEK_SET) < 0) return ERR_DB_FILE;

    char zero_block[64] = {0};
    read(fd, s, STUDENT_RECORD_SIZE);
    if (memcmp(s, zero_block, STUDENT_RECORD_SIZE) == 0) return SRCH_NOT_FOUND;
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t student = {0};
    if (get_student(fd, id, &student) == 0) {
    ...
    off_t offset_result = lseek(fd, offset, SEEK_SET);
    if (offset_result < 0) {
...
int del_student(int fd, int id)
{
    ...
    student_t s2 = EMPTY_STUDENT_RECORD;
    if (get_student(fd, id, &s) == -3) {
        ...
        if (write(fd, &s2, STUDENT_RECORD_SIZE) < 0) {
            printf(M_ERR_DB_WRITE);
            return ERR_DB_FILE;
        }
    }
...
int count_db_records(int fd)
{
    ...
    while (bytes_read != 0) {
        if (memcmp(student, zero_block, STUDENT_RECORD_SIZE) != 0) {
            student_count++;
        }
        bytes_read = read(fd, student, STUDENT_RECORD_SIZE);
    }
    if (student_count == 0) printf(M_DB_EMPTY);
    else printf(M_DB_RECORD_CNT, student_count);
...
int print_db(int fd)
{
    ...
    if (header_printed == 0) printf(M_DB_EMPTY);
    return NO_ERROR;
}
